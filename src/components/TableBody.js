...
const TableBody = (contact, direction, deleteContact, listNumber, shiftContact) => {
    ...
    return(
            <tr key={contact.id}>
                {direction &&
                    <Fragment>
                        <td><Button color="primary" onClick={()=> shiftContact(contact.id, listNumber)}><Octicon icon={ArrowLeft}/></Button></td>
                ...
                {!direction &&
                    <Fragment>
                        <td><Button color="danger" onClick={()=> deleteContact(contact.id, listNumber)}><Octicon icon={Trashcan}/></Button></td>
                        <td><Button color="primary" onClick={()=> shiftContact(contact.id, listNumber)}><Octicon icon={ArrowRight}/></Button></td>
                    </Fragment>
            }
            </tr>
        ) 
